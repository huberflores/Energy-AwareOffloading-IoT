package ee.ut.mobile.contextaware.neural.accely;
...
import static ee.ut.mobile.contextaware.neural.util.Constant.ACCELY_ID;
import static ee.ut.mobile.contextaware.neural.util.Constant.BATTERY_ID;


public class SVMAccelyTrainer {
    ...
    private double[] normalizedAccely;
    private List<Timestamp> timing;
    private double[] closedLoopAccely;
    ...
    public void normalizeAccelyWithTimeBetween(double lo, double hi, Timestamp start, Timestamp stop) {
        bws = DAOManager.getDataWithTimeBetween(DataSourceFactory.getDataSource(), ACCELY_ID, start, stop);
        ...
        normalizedAccely = norm.process(tempPrimitiveArray);
        closedLoopAccely = EngineArray.arrayCopy(normalizedAccely);
    }
    ...
    public MLDataSet generateTraining() {
        ...
        temp.analyze(this.normalizedAccely);
        return temp.process(this.normalizedAccely);
    }

    public SVM createNetwork() {
        SVM network = new SVM(WINDOW_SIZE, true);
        network.getParams().eps = 3 * stdDeviation  * Math.sqrt(Math.log(normalizedAccely.length)/normalizedAccely.length);
    ...
    public void predict(SVM network) {
        ...
        for (int evalutingpoint = start; evalutingpoint < timing.size(); evalutingpoint++) {
            ...
            for (int i = 0; i < input.size(); i++) {
                input.setData(i, this.normalizedAccely[(evalutingpoint - WINDOW_SIZE) + i]);
            }

            MLData output = network.compute(input);
            double prediction = output.getData(0);
            this.closedLoopAccely[evalutingpoint] = prediction;

            // calculate "closed loop", based on predicted data
            for (int i = 0; i < input.size(); i++) {
                input.setData(i, this.closedLoopAccely[(evalutingpoint - WINDOW_SIZE) + i]);
            }
            ...
            //ErrorCalculation
            ec.updateError(prediction, normalizedAccely[evalutingpoint]);


            // display
            System.out.println(timing.get(evalutingpoint) + "\t"
                    + f.format(norm.getStats().deNormalize(this.normalizedAccely[evalutingpoint])) + "\t"
    ...
    public void run() {
        ...
        normalizeAccelyWithTimeBetween(0.1, 0.9, new Timestamp(start.getTime().getTime()),
    ...
    public static void main(String[] args) {
        SVMAccelyTrainer SVMAccelyTrainer = new SVMAccelyTrainer();
        SVMAccelyTrainer.run();

...
public class SVMTempTrainer {
    ...
    private double[] closedLoopTemperature;
    ...
    public void normalizeTemperatureWithTimeBetween(double lo, double hi, Timestamp start, Timestamp stop) {
        ...
        closedLoopTemperature = EngineArray.arrayCopy(normalizedTemperature);
    }
    ...
    public MLDataSet generateTraining() {
        ...
        temp.analyze(this.normalizedTemperature);
        return temp.process(this.normalizedTemperature);
    }

    public SVM createNetwork() {
        SVM network = new SVM(WINDOW_SIZE, true);
        network.getParams().eps = 3 * stdDeviation  * Math.sqrt(Math.log(normalizedTemperature.length)/normalizedTemperature.length);
    ...
    public void predict(SVM network) {
        ...
        for (int evalutingpoint = start; evalutingpoint < timing.size(); evalutingpoint++) {
            ...
            for (int i = 0; i < input.size(); i++) {
                input.setData(i, this.normalizedTemperature[(evalutingpoint - WINDOW_SIZE) + i]);
            }

            MLData output = network.compute(input);
            double prediction = output.getData(0);
            this.closedLoopTemperature[evalutingpoint] = prediction;

            // calculate "closed loop", based on predicted data
            for (int i = 0; i < input.size(); i++) {
                input.setData(i, this.closedLoopTemperature[(evalutingpoint - WINDOW_SIZE) + i]);
            }
            ...
            //ErrorCalculation
            ec.updateError(prediction,normalizedTemperature[evalutingpoint]);

            // display
            System.out.println(timing.get(evalutingpoint) + "\t"
                    + f.format(norm.getStats().deNormalize(this.normalizedTemperature[evalutingpoint])) + "\t"
    ...
    public static void main(String[] args) {
        SVMTempTrainer svmTempTrainer = new SVMTempTrainer();
        svmTempTrainer.run();

...
public class NeatAccelyTrainer {
    ...
    public void normalizeAccelyWithTimeBetween(double lo, double hi, Timestamp start, Timestamp stop) {
        accelyz = DAOManager.getDataWithTimeBetween(DataSourceFactory.getDataSource(), BATTERY_ID, start, stop);

        size = accelyz.size();

        Set<Timestamp> times = accelyz.keySet();

        startingTime = ((Timestamp) ((TreeMap) accelyz).firstKey()).getTime();
        startingTimeStamp = ((Timestamp) ((TreeMap) accelyz).firstKey());
        lastTime = ((Timestamp) ((TreeMap) accelyz).lastKey()).getTime();
        lastTimeStamp = (Timestamp) ((TreeMap) accelyz).lastKey();


        DescriptiveStatistics destats = new DescriptiveStatistics();

        for (Timestamp t : times) {
            destats.addValue(accelyz.get(t));
        }
    ...
    public List<Double> addMissingData() {
        ...
        while (newTimeStamp.before(lastTimeStamp)) {
            temperatures.add(accelyz.get(newTimeStamp) != null ? accelyz.get(newTimeStamp) : median);
    ...
    public static void main(String[] args) {
        NeatAccelyTrainer neatAccelyTrainer = new NeatAccelyTrainer();
        neatAccelyTrainer.run();

...
public class SVMBwTrainer {
    ...
    public void normalizeBwWithTimeBetween(double lo, double hi, Timestamp start, Timestamp stop) {
        bws = DAOManager.getDataWithTimeBetween(DataSourceFactory.getDataSource(), BW_ID, start, stop);
        ...
        normalizedBw = norm.process(tempPrimitiveArray);
        closedLoopBattery = EngineArray.arrayCopy(normalizedBw);
    }
    ...
    public MLDataSet generateTraining() {
        ...
        temp.analyze(this.normalizedBw);
        return temp.process(this.normalizedBw);
    }

    public SVM createNetwork() {
        SVM network = new SVM(WINDOW_SIZE, true);
        network.getParams().eps = 3 * stdDeviation  * Math.sqrt(Math.log(normalizedBw.length)/normalizedBw.length);
    ...
    public void predict(SVM network) {
        ...
        for (int evalutingpoint = start; evalutingpoint < timing.size(); evalutingpoint++) {
            ...
            for (int i = 0; i < input.size(); i++) {
                input.setData(i, this.normalizedBw[(evalutingpoint - WINDOW_SIZE) + i]);
            }
            ...
            //ErrorCalculation
            ec.updateError(prediction,normalizedBw[evalutingpoint]);



            // display
            System.out.println(timing.get(evalutingpoint) + "\t"
                    + f.format(norm.getStats().deNormalize(this.normalizedBw[evalutingpoint])) + "\t"
    ...
    public void run() {
        ...
        later.set(2014, Calendar.MAY, 17, 20, 50, 02);

        normalizeBwWithTimeBetween(0.1, 0.9, new Timestamp(start.getTime().getTime()),
    ...
    public static void main(String[] args) {
        SVMBwTrainer SVMBwTrainer = new SVMBwTrainer();
        SVMBwTrainer.run();

package ee.ut.mobile.contextaware.neural.battery;
...
public class BasicBatteryTrainer {
    ...
    private double[] normalizedHumidity;
    private List<Timestamp> timing;
    private double[] closedLoopHumidity;
    private Timestamp lastTimeStamp;
    NormalizeArray norm;
    private double median;

    public void normalizeHumidityWithTimeBetween(double lo, double hi, Timestamp start, Timestamp stop) {
        ...
        normalizedHumidity = norm.process(tempPrimitiveArray);
        closedLoopHumidity = EngineArray.arrayCopy(normalizedHumidity);
    ...
    public MLDataSet generateTraining() {
        ...
        temp.analyze(this.normalizedHumidity);
        return temp.process(this.normalizedHumidity);
    }
    ...
    public void predict(BasicNetwork network) {
        ...
        for (int evalutingpoint = start; evalutingpoint < timing.size(); evalutingpoint++) {
            ...
            for (int i = 0; i < input.size(); i++) {
                input.setData(i, this.normalizedHumidity[(evalutingpoint - WINDOW_SIZE) + i]);
            }

            MLData output = network.compute(input);
            double prediction = output.getData(0);
            this.closedLoopHumidity[evalutingpoint] = prediction;

            // calculate "closed loop", based on predicted data
            for (int i = 0; i < input.size(); i++) {
                input.setData(i, this.closedLoopHumidity[(evalutingpoint - WINDOW_SIZE) + i]);
            }
            ...
            //ErrorCalculation
            ec.updateError(prediction,normalizedHumidity[evalutingpoint]);

            // display
            System.out.println(timing.get(evalutingpoint) + "\t"
                    + f.format(norm.getStats().deNormalize(this.normalizedHumidity[evalutingpoint])) + "\t"
    ...
    public void run() {
        ...
        normalizeHumidityWithTimeBetween(0.1, 0.9, new Timestamp(start.getTime().getTime()),
    ...
    public static void main(String[] args) {
        BasicBatteryTrainer basicBatteryTrainer = new BasicBatteryTrainer();
        basicBatteryTrainer.run();

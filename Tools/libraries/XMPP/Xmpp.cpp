...
bool XMPP::connect() {
	...
	while((millis() - currentTime) < 1000 * 20) { // Less than 20 seconds... then time out
		if(sendMessage){
			sendStanza();
			sendMessage = false;
		}
...
void XMPP::handleIncoming() {
	...
	do {
		...
		if(dataReceived && checkIfComplete(buffer)) {
			debug("handleIncoming Complete message received");
			dataComplete = true;
			break;
		}
	...
	if(dataReceived){
		debug("HandeIncoming Incoming", buffer);
		currentLetter = 0;
		processInput(buffer);
        debug("ProcessInput done");
		flushBuffer();
	}
}
...
bool XMPP::handleOpenStream(String input) {
	...
	if(input.indexOf("?xml") > -1){
		input2 = input.substring(input.indexOf("<", 1), input.length() + 1);
	}
	if(input.indexOf("stream:features") > -1) {
	    ...
	    debug("stream:features not found");
	    debug("original input");
	    char inChars[input.length()];
	    input.toCharArray(inChars, input.length());
	    debug(inChars);

	    debug("modified input");
	    char inChars2[input2.length()];
        input2.toCharArray(inChars2, input2.length());
        debug(inChars2);
	}
	return false;
}
...
void XMPP::handlePresence(String input) {
	...
	if(type.length() > 0) {
        ...
        debug("show body value");
        debug(status);
		String from = findAttrValue(input, "from");
        debug("from attr value");
        debug(from);
		if(from.startsWith(this->recipient)) {
...
void XMPP::debug(String message) {
	if(this->serial != NULL) {
